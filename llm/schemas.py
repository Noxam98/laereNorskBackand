...
LANG_NAMES = {"ru": "русский", "ukr": "украинский", "en": "английский", "pl": "польский", "lt": "литовский", "lv": "латышский"}
...
DEDUP_SCHEMA = {
    ...
    "schema": {
        ...
        "properties": {
            # отношение пары:
            #  same_word      — одно слово в разных написаниях (орфография/диакритика/диалект/опечатка),
            #                   значение и часть речи идентичны;
            #  inflected_form — одно из них ГРАММАТИЧЕСКАЯ ФОРМА другого (спряжение, опред./мн. форма,
            #                   склонение): bærer←bære, dyret←dyr, omgivelser←omgivelse;
            #  different      — разные слова, синонимы или нет уверенности.
            "relation": {"type": "string", "enum": ["same_word", "inflected_form", "different"]},
            # словарная ЛЕММА, которую нужно ОСТАВИТЬ (инфинитив / ед.ч. неопр. / базовая форма) —
            # ровно A или B; пусто если different.
            "canonical": {"type": "string"},
        },
        "required": ["relation", "canonical"],
    },
}
