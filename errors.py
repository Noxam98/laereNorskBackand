...
def classify(exc) -> LlmError:
    ...
    if is_(RateLimitError) or status == 429 \
            or "resource_exhausted" in low or "quota" in low or "rate limit" in low:
    ...
    # 3) Таймаут
    if is_(APITimeoutError) or "timed out" in low or "timeout" in low:
        return LlmError(TIMEOUT, 504, "Translation provider timeout", True,
                        "Таймаут запроса к Gemini")
    # 4) Сеть / соединение
    if is_(APIConnectionError) or "connection" in low:
    ...
    if (status and status >= 500) or "internal server" in low:
        return LlmError(SERVER, 502, "Translation provider error", True,
                        f"Сбой провайдера Gemini ({status or '5xx'})")
    # 6) Плохой запрос (наша ошибка/неподдерживаемая фича) — не алёртим, это не инцидент
