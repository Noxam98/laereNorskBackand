...
from morphology import is_irregular_noun, regular_indef_pl   # морфология сущ. — для грамм-клеток (род/мн.ч.)
...
def _grammar_cells(norwegian, data, forms):
    ...
    «выучено» и CEFR (отдельный слой, см. apply_result). Сейчас (NOUN-срез): существительное →
    choice_gender (есть род en/ei/et) + input_indefpl (мн.ч. ЕСТЬ и оно нерегулярно — иначе
    пользователь выводит его правилом, проверять нечего)."""
    try:
        d = json.loads(data) if isinstance(data, str) else (data or {})
    except Exception:
        d = {}
    if normalize_pos(d.get("part_of_speech")) != "noun":
        return []
    try:
    ...
    # input проверяем ТОЛЬКО при валидном роде (предсказание правила опирается на род — без него
    # нельзя достоверно судить о регулярности, иначе ложный дрилл) и сравниваем в NFC (форма из БД
    # может быть в NFD → иначе ложное «нерегулярно» + цель, которую не ввести с обычной раскладки).
    if indef_pl and gender in _GENDERS:
        ip = unicodedata.normalize("NFC", indef_pl).lower()
        reg = regular_indef_pl(norwegian or "", gender)        # предсказание по правилу
        if reg:
            if ip != unicodedata.normalize("NFC", reg.strip()).lower():   # факт ≠ правило → форму надо знать
                out.append("input_indefpl")
        elif is_irregular_noun(norwegian or "", gender, f)[0]:  # правило не предсказало → детектор
            out.append("input_indefpl")
    return out
...
def _grammar_element(row, cell, forms, data):
    ...
    base = {
        ...
        "part_of_speech": "noun", "forms": forms,
    ...
    if cell == "input_indefpl":
        return {**base, "mode": "input", "direction": "indefpl",
                "target": {"field": "indef_pl", "value": (forms.get("indef_pl") or "").strip()},
                "prompt": {"kind": "lemma+formLabel", "formLabel": "indef_pl", "lemma": no},
