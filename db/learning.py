...
async def _activity_metrics(db, user_id):
    ...
    # ключи журнала пишутся по UTC-дате (_now()[:10], _now=utcnow) — читаем той же UTC-датой (_today),
    # иначе на не-UTC сервере дневная цель/стрик ломались бы на стыке суток (локальная date.today()).
    today = _today()
...
_USER_LOCKS_CAP = 512   # порог опортунистической чистки реестра (иначе он растёт по локу на юзера навсегда)


def _user_lock(user_id: int) -> asyncio.Lock:
    lk = _USER_LOCKS.get(user_id)
    if lk is None:
        # Опортунистическая эвикция: реестр никогда не чистился (лок на каждого юзера навсегда).
        # Когда он раздулся И мы всё равно создаём новый лок — выметаем «тихие» записи (не захвачены и
        # без ожидающих). Event loop однопоточный, между проверкой и pop нет await → удерживаемый/
        # ожидаемый лок удалить нельзя (корректность сохранена); лениво-создаём как раньше.
        if len(_USER_LOCKS) > _USER_LOCKS_CAP:
            for uid, other in list(_USER_LOCKS.items()):
                if not other.locked() and not other._waiters:
...
async def _apply_result_inner(user_id: int, pool_id: int, correct: bool, elapsed: float = None,
    ...
    try:
        ...
        if cell and cell in gcells and cell not in cells:
            ...
            # строка user_words могла исчезнуть между сборкой сессии и ответом (learning_remove и т.п.):
            # overlay-путь только UPDATE → прогресс формы «в пустоту». Гарантируем строку (как set_status).
            await db.execute("INSERT OR IGNORE INTO user_words (user_id, pool_id, created_at) VALUES (?,?,?)",
        ...
        # (зеркалит ленивый fetch audio_on выше). note_cycle_mastered остаётся в транзакции — пишем его
        # уже под открытым локом, но БЕЗ лишних cross-conn чтений внутри.
        should_note_cycle = False
        if mastered_now and not (st.get("mastered") or 0) and wp and wp["forms"]:
            try:
                _d0 = json.loads(wp["data"]) if wp["data"] else {}
            except Exception:
                _d0 = {}
            _pos0 = normalize_pos(_d0.get("part_of_speech"))
            from .learning_forms import is_formable
            # Гейт per-POS тумблером — ТОТ ЖЕ критерий, что у build_universe (форм-фаза): иначе
            # выключенный тумблером POS копится в батч, но выпадает из плана → цикл churn'ит forms↔words.
            if is_formable(_pos0, wp["forms"], wp["norwegian"]):
                grp = _GRAMMAR_GROUP.get(_pos0)
                if grp:
                    from .users import get_user_grammar, get_user_grammar_pos
                    if await get_user_grammar(user_id):
                        gpos = await get_user_grammar_pos(user_id)
                        should_note_cycle = bool(gpos.get(grp, True))
        await db.execute("""
        ...
        if mastered_now and not (st.get("mastered") or 0):
            ...
            # цикл «слова↔формы»: формо-способное свежевыученное — в копилку партии (решение о том,
            # копить ли, посчитано ДО upsert'а в should_note_cycle, чтобы не читать настройки под локом)
            if should_note_cycle:
                from .learning_forms import note_cycle_mastered
...
async def listen_status(user_id):
    """Сколько слов «ждёт слух» + порог партии + готова ли слуховая сессия.
    Готовность: pending >= pack ИЛИ (pending > 0 И приток новых иссяк). Второе — «хвост»: когда
    больше нечему дорасти в пачку, а несколько слов ждут слух, партия иначе НИКОГДА не наберёт pack
    и слова зависнут «невыучиваемыми» (build_listen_session их и так обслужит). Пока есть чему дорасти
    (growable > 0) — порог прежний (pack), рано слух не открываем.
    ...
    try:
        ...
        await _release(db)
    pending = 0
    growable = 0   # контентные слова, ещё способные ДОрасти в слуховую пачку (не выучены/не служебные,
                   # текстовую рампу ещё не сдали, вкл. свои новые/недобавленные) → пачка ещё может расти
    for r in rows:
        try:
            modes = json.loads(r.get("modes") or "{}")
        except Exception:
        ...
        if _is_audio_pending(r, modes, data):
            ...
            continue
        st = status_of(r, modes)
        if st in ("new", "learning", "review", "weak") and not is_function_word(r["norwegian"], data):
            growable += 1
    intake_exhausted = growable == 0
    ready = pending >= pack or (pending > 0 and intake_exhausted)
    return {"pending": pending, "pack": pack, "ready": ready, "audio": True}
