...
def _vis_cond(user_id):
    """Видимость модерации для запросов пользователя: общая база (approved=1) + личное
    расширение автора (его pending/rejected). Без user_id (внутренние вызовы) — только общая
    база. Возвращает (cond, params). Единое правило для get_pool_list/search_pool."""
    if user_id:
        return "(COALESCE(approved,1) = 1 OR created_by = ?)", [user_id]
...
async def search_pool(prefix: str, limit: int = 10, lang: str = None, user_id: int = None):
    ...
    ВИДИМОСТЬ МОДЕРАЦИИ обязательна (как в get_pool_list): без неё автокомплит отдавал чужие
    неодобренные слова (approved=0) с inPool=True — их существование утекало, а кнопка
    «Показать» в Базе загоралась на карточку, которой пользователь не видит."""
    from db import ordbank
    key = normalize_word(prefix)
    if not key:
        return []
    limit = max(1, min(50, limit))   # защита: limit<1 → LIMIT -1 = без ограничения (весь пул), кап сверху
    vis, vparams = _vis_cond(user_id)
    db = await _conn()
    try:
        out, have = [], set()
        ...
        async with db.execute(
            f"SELECT norwegian, data FROM word_pool "
            f"WHERE (norwegian LIKE ? OR data LIKE ?) AND {vis} "
            "ORDER BY (CASE WHEN norwegian = ? THEN 0 WHEN norwegian LIKE ? THEN 1 ELSE 2 END), "
            "norwegian LIMIT ?",
            (key + "%", "%" + key + "%", *vparams, key, key + "%", limit),
        ) as cur:
            for r in await cur.fetchall():
        ...
        for form, lemma, pos in cands:
            if len(out) >= limit or lemma in have:
                continue
            async with db.execute(
                f"SELECT norwegian, data FROM word_pool WHERE norwegian = ? AND pos = ? AND {vis}",
                (lemma, pos, *vparams)) as cur:
                r = await cur.fetchone()
            if r:
                have.add(r["norwegian"].lower())
        ...
        if len(key) >= 3 and len(out) < limit:
            ...
            if fids:
                marks = ",".join("?" for _ in fids)
                async with db.execute(
                        f"SELECT id, norwegian, data FROM word_pool WHERE id IN ({marks}) AND {vis}",
                        (*fids, *vparams)) as curf:
