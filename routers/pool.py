...
async def pool_generate(body: dict, user=Depends(llm_rate_limit)):
    ...
    mark_activity()
    # Единый путь с импортом: Ordbank сводит форму к лемме, Lexin даёт человеческий перевод,
    # а сохранение/эмбеддинги выполняются пакетным примитивом генератора.
    from autofill import resolve_bank_words
    bank = await resolve_bank_words([{"word": word, "translation": ""}], created_by=user["id"])
    if bank:
        resolved = bank[0]
        if resolved["created"]:
        ...
        out = {
            "word": resolved["word"],
            "pool_id": resolved["pid"],
            "generated": resolved["created"],
            "source": resolved["source"],
            "translate": resolved["translate"],
        }
        if resolved["via_form"]:
            out["viaForm"] = resolved["via_form"]
        return out
...
async def pool_meta(word: str, pool_id: int = None, lang: str = None, user=Depends(get_current_user)):
    """Темы и уровень слова (для показа в карточке). pool_id — точная запись омонима (напр. `ro`
    сущ./глаг.), иначе первая по norwegian. lang — язык переводов в дереве разбора композита."""
    lg = lang if lang in LANG_SET else "ru"
    meta = await get_pool_meta(word, user_id=user["id"], pool_id=pool_id, lang=lg)
